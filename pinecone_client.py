        ...
        try:
            ...
            for chunk in chunks:
                ...
                pinecone_metadata = {
                    ...
                    # REMOVED: Text field is stored at top level of Pinecone record (not in metadata)
                    # This prevents metadata size limit errors (40KB limit) since text can be 40KB+ alone
                    # Text is accessible via match.text attribute in search results
                }
                # TOTAL: 29 fields (text removed from metadata - stored at top level)
    ...
    def hybrid_search_documents(
        ...
        try:
            ...
            for match in all_results:
                ...
                # Text comes from top level of Pinecone record, not metadata
                # Pinecone automatically returns text at top level via match.text attribute
                ...
                try:
                    ...
                    for match in results.matches:
                        all_results.append(DocumentSearchResult(
                            ...
                            metadata=match.metadata,
                            text=getattr(match, 'text', '') if include_metadata else ""  # Text from top level, not metadata
                        ))
                except Exception as e:
